  ...
  return declare("bgagame.thecrew", ebg.core.gamegui, {
    ...
    setup: function (gamedatas) {
      ...
      for (var player_id in gamedatas.players) {
        ...
        if (cardontable !== undefined) {
          dojo.place(
            this.format_block("jstpl_cardontable", cardontable),
            $("playertablecard_" + this.players[player_id]["relative"]),
          );
          this.createCardTooltip(
            "card_" + cardontable["id"],
            cardontable["type"],
            cardontable["type_arg"],
          );
        }
        ...
        if (cardontable !== undefined) {
          ...
          dojo
            .query("#card_" + cardontable["id"])
            .connect("onclick", this, "onPlayCard");
          this.createCardTooltip(
            "card_" + cardontable["id"],
            cardontable["type"],
            cardontable["type_arg"],
        ...
        dojo.place(
          this.format_block("jstpl_player", { player_id: player_id }),
        ...
        this.cards_counters[player_id].create(
          "cardsinhands_counter_" + player_id,
        );
        this.cards_counters[player_id].setValue(player["cards_number"]);

        this.addCustomTooltip(
          "tricks_" + player_id,
          _("Number of tricks won."),
        );
        this.addCustomTooltip(
          "cardsinhand_" + player_id,
          _("Number of cards in hand."),
        );

        var commander_desc = _(
          "The commander is always the player with the four rocket. <br/>The duties of the commander are: <br/>1. start the selection of the tasks <br/>2. start the first trick <br/>3. implement the special rules of individual missions",
        );
        this.addTooltipHtml(
          "commander_in_panel_" + player_id,
          this.format_block("jstpl_tooltip_common", {
            title: _("Commander token"),
            description: commander_desc,
          }),
        );
        this.addTooltipHtml(
          "commander_icon_spot_" + player_id,
          this.format_block("jstpl_tooltip_common", {
            title: _("Commander token"),
            description: commander_desc,
          }),
        ...
        this.addTooltipHtml(
          "special_icon_spot_" + player_id,
          this.format_block("jstpl_tooltip_common", {
            title: _("Special one"),
            description: _("This crew member is special for this mission."),
          }),
        );
        this.addTooltipHtml(
          "special2_icon_spot_" + player_id,
          this.format_block("jstpl_tooltip_common", {
            title: _("Second special one"),
            description: _("This crew member must win the last trick."),
          }),
        );
        ...
        this.addTooltipHtml(
          "radio_" + player_id,
          this.format_block("jstpl_tooltip_common", {
            title: _("Radio communication token"),
            description: _(
              "Communication token gives information of the communicated color card :<br/>- At the top, if it is your highest card of this color.<br/>- In the middle, if it is your only card of this color.<br/>- At the bottom, if it is your lowest card of this color.<br/>- Red, you cannot communicate.",
            ),
          }),
        );
      }
      ...
      dojo
        .query("#commander_icon_spot_" + gamedatas.commander_id)
        .removeClass("hidden");
      dojo.query(".commander_in_panel").addClass("hidden");
      dojo
        .query("#commander_in_panel_" + gamedatas.commander_id)
        .removeClass("hidden");
      dojo.query(".special").addClass("hidden");
      dojo
        .query("#special_icon_spot_" + gamedatas.special_id)
        .removeClass("hidden");
      dojo.query(".special2").addClass("hidden");
      dojo
        .query("#special2_icon_spot_" + gamedatas.special2_id)
        .removeClass("hidden");
      ...
      for (var task_id in this.gamedatas.tasks) {
        ...
        dojo.place(
          this.format_block("jstpl_task", task),
          $("tasks_" + task["player_id"]),
        );
      ...
      this.addTooltipHtml(
        "distress",
        this.format_block("jstpl_tooltip_common", {
          title: _("Distress token"),
          description: _(
            "A distress signal can be sent out before the first trick of a mission and before any communication. If the distress signal is activated, each crew member may pass one card to his neighbor. Rockets may not be passed on!",
          ),
        }),
      );
      ...
      switch (stateName) {
        case "pickTask":
          ...
          if (this.isCurrentPlayerActive()) {
            ...
            dojo
              .query("#tasklists .taskontable")
              .connect("onclick", this, "onChooseTask");
          }
        ...
        case "playerTurn":
          if (this.isCurrentPlayerActive()) {
            console.log(args.args.cards);
            for (var card_id in args.args.cards) {
              dojo
                .query("#myhand_item_" + args.args.cards[card_id])
                .addClass("selectable");
              dojo
                .query("#card_" + args.args.cards[card_id])
                .addClass("selectable");
            }
          }
          if (
            !this.isSpectator &&
            args.args.players[this.player_id]["canCommunicate"]
          ) {
            $("pagemaintitletext").innerHTML =
              $("pagemaintitletext").innerHTML +
              _(" or {you} can communicate").replace("{you}", this.divYou());
            dojo.query("#comcard_" + this.player_id).addClass("selectable");
          } else {
            dojo.query("#comcard_" + this.player_id).removeClass("selectable");
          }
        ...
        case "commToken":
          if (this.isCurrentPlayerActive()) {
            for (var status_id in args.args.possible) {
              ...
              dojo.place(
                this.format_block("jstpl_temp_comm", {
                  player_id: this.player_id,
                  status: status,
                }),
                $("comcard_" + this.player_id),
                "first",
              );
            }
        ...
        case "endMission":
          ...
          if (args.args.end > 0) {
            $("endResult").innerHTML = _(
              'Mission ${nb} <span class="success">completed</span>',
            ).replace("${nb}", args.args.number);
          } else {
            $("endResult").innerHTML = _(
              'Mission ${nb} <span class="failure">failed</span>',
            ).replace("${nb}", args.args.number);
          }
          if (!this.isCurrentPlayerActive()) {
            dojo.query(".finalbutton").addClass("hidden");
            dojo.query("#continue_ok").removeClass("hidden");
          } else {
            dojo.query(".finalbutton").removeClass("hidden");
            dojo.query("#continue_ok").addClass("hidden");
          }
          break;
    ...
    onButtonChoose: function (event) {
      ...
      if (this.isCurrentPlayerActive() && this.checkAction("actButton")) {
        ...
        this.ajaxcall(
          "/thecrew/thecrew/actButton.html",
          {
            lock: true,
            choice: $action,
          },
    ...
    onDistress: function (event) {
      ...
      if (event.currentTarget.classList.contains("selectable")) {
        this.confirmationDialog(
          _("Are you sure you want to launch a distress signal?"),
          dojo.hitch(this, function () {
            dojo.query(".selectable").removeClass("selectable");

            this.ajaxcall(
              "/thecrew/thecrew/actDistress.html",
              {
                lock: true,
              },
              this,
    ...
    onCancel: function (event) {
      ...
      if (this.checkAction("actCancel")) {
        dojo.query(".selectable").removeClass("selectable");
        this.ajaxcall(
          "/thecrew/thecrew/actCancel.html",
          {
            lock: true,
          },
          this,
    ...
    onStartComm: function (event) {
      ...
      if (event.currentTarget.classList.contains("selectable")) {
        ...
        this.ajaxcall(
          "/thecrew/thecrew/actStartComm.html",
          {
            lock: true,
          },
          this,
      ...
      if (event.currentTarget.classList.contains("selectable")) {
        ...
        var place =
          event.currentTarget.classList[
            event.currentTarget.classList.length - 1
          ];

        this.ajaxcall(
          "/thecrew/thecrew/actFinishComm.html",
          {
            lock: true,
            place: place,
          },
    ...
    onPickCrew: function (event) {
      ...
      if (this.isCurrentPlayerActive()) {
        if (
          event.currentTarget.classList.contains("selectable") &&
          this.checkAction("actPickCrew")
        ) {
          ...
          this.ajaxcall(
            "/thecrew/thecrew/actPickCrew.html",
            {
              lock: true,
              crewId: id,
            },
    ...
    onMultiSelect: function (event) {
      ...
      if (this.isCurrentPlayerActive() && this.stateName == "multiSelect") {
        if (event.currentTarget.classList.contains("selected")) {
          ...
          event.currentTarget.classList.contains("selectable") &&
          this.checkAction("actMultiSelect")
        ) {
          ...
          if (this.selected == null) {
            ...
            dojo.query(".selectable").removeClass("selectable");
            this.ajaxcall(
              "/thecrew/thecrew/actMultiSelect.html",
              {
                lock: true,
                id1: this.selected,
                id2: id,
    ...
    onPlayCard: function (event) {
      ...
      if (this.isCurrentPlayerActive()) {
        if (
          ...
          this.ajaxcall(
            "/thecrew/thecrew/actPlayCard.html",
            {
              lock: true,
              cardId: id,
            },
    ...
    onChooseTask: function (event) {
      ...
      if (this.isCurrentPlayerActive()) {
        if (
          event.currentTarget.classList.contains("selectable") &&
          this.checkAction("actChooseTask")
        ) {
          ...
          this.ajaxcall(
            "/thecrew/thecrew/actChooseTask.html",
            {
              lock: true,
              taskId: id,
            },
    ...
    symbol: function (name) {
      return dojo.string.substitute("<span class='logicon ${name}'></span>", {
        name: name,
      });
    },
    ...
    shapeTooltip: function (help_HTML, action_HTML) {
      return this.format_string_recursive("${content}", { content: help_HTML });
    },
    ...
    addCustomTooltip: function (nodeId, help_HTML, action_HTML, delay) {
      ...
      this.addTooltipHtml(
        nodeId,
        this.shapeTooltip(help_HTML, action_HTML),
        delay,
      );
    },

    addCustomTooltipToClass: function (
      cssClass,
      help_HTML,
      action_HTML,
      delay,
    ...
    createCardTooltip: function (card_HTML_id, color, value) {
      if (color == 6) {
        ...
        this.addTooltipHtml(
          card_HTML_id,
          this.format_block("jstpl_tooltip_common", {
            title: _("Reminder card"),
            description: _(
              "On table, it allows you to start communication by clicking on it <b>before</b> a trick. <br/>In hand, its purpose is to remind you that your communicated card is still on the table.",
            ),
          }),
        );
      } else {
        var msg = this.format_string_recursive(
          _("${value_symbol}${color_symbol} : ${value_name} ${color_name}."),
          {
            value_name: value,
            color_name: color,
            value_symbol: value,
        ...
        this.addCustomTooltip(
          ...
          dojo.string.substitute(
            '<span class="card_description">${msg}</span>',
            { msg: msg },
          ),
        );
      }
    ...
    createTaskTooltip: function (task) {
      ...
      var msg = this.format_string_recursive(
        _(
          "<b>Task</b><br/> ${value_symbol}${color_symbol} : ${value_name} ${color_name}.",
        ),
        {
          value_name: value,
          color_name: color,
      ...
      this.addCustomTooltip(
        card_HTML_id,
        dojo.string.substitute('<span class="card_description">${msg}</span>', {
          msg: msg,
        }),
      );
    },
    ...
    notif_giveAllCardsToPlayer: function (notif) {
      ...
      for (var card_id in notif.args.cards) {
        ...
        var anim = this.slideToObject(
          "card_" + card["id"],
          "playertablecard_" + this.players[winner_id]["relative"],
          1000,
        ); // Program the anim: 1. Slide the cart (1 second)
        if (b_winning_card) {
          self = this;
          dojo.connect(anim, "onEnd", function (node) {
            self.fadeOutAndDestroy(node, 1000);
    ...
    notif_taskUpdate: function (notif) {
      ...
      dojo.addClass(
        "status_" + notif.args.task["task_id"],
        notif.args.task["status"],
      );
    },
    ...
    notif_commander: function (notif) {
      dojo.query(".commander").addClass("hidden");
      dojo
        .query("#commander_icon_spot_" + notif.args.player_id)
        .removeClass("hidden");
      dojo.query(".commander_in_panel").addClass("hidden");
      dojo
        .query("#commander_in_panel_" + notif.args.player_id)
        .removeClass("hidden");
    },
    ...
    notif_special: function (notif) {
      if (notif.args.special2 == true) {
        dojo.query(".special2").addClass("hidden");
        dojo
          .query("#special2_icon_spot_" + notif.args.player_id)
          .removeClass("hidden");
      } else {
        dojo.query(".special").addClass("hidden");
        dojo
          .query("#special_icon_spot_" + notif.args.player_id)
          .removeClass("hidden");
      }
    },
    ...
    notif_commCard: function (notif) {
      ...
      if (player_id == this.player_id) {
        this.player_hand.addToStockWithId(
          this.getCardUniqueId(6, 0),
          notif.args.reminder_id,
        );
        dojo
          .query("#myhand_item_" + notif.args.reminder_id)
          .connect("onclick", this, "onPlayCard");
      }
    ...
    notif_takeTask: function (notif) {
      ...
      if ($(taskId).classList.contains("col7")) {
        ...
        dojo
          .query("#" + taskId)
          .addClass("col" + task["card_type"] + " val" + task["card_type_arg"]);
      }
      this.createTaskTooltip(task);
    ...
    notif_cleanUp: function (notif) {
      ...
      for (var player_id in this.players) {
        ...
        this.cards_counters[player_id].setValue(
          notif.args.players[player_id]["nbCards"],
        );
    ...
    playCardOnTable: function (card, comm = false) {
      ...
      if (preexist) {
        ...
        var test = this.format_block("jstpl_cardontable", card);
        dojo.place(test, $(target));
        dojo
          .query("#card_" + card["id"])
          .connect("onclick", this, "onPlayCard");
      }
      ...
      try {
        if (log && args && !args.processed) {
          ...
          if (args.value_symbol !== undefined) {
            args.value_symbol = dojo.string.substitute(
              "<strong class='${actual_color}'>${value_symbol}</strong>",
              {
                actual_color: this.colors[args.color_symbol].color,
                value_symbol: args.value_symbol,
              },
            );
          }
          ...
          if (args.color_symbol !== undefined) {
            args.color_symbol = dojo.string.substitute(
              "<span class='logicon ${actual_color}' title='${actual_color_name}'></span>",
              {
                actual_color: this.colors[args.color_symbol].color,
                actual_color_name: this.colors[args.color_symbol].name,
              },
            );
          }

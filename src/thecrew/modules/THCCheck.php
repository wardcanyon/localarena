...
class THCCheck
{
    ...
    function check()
    {
        ...
        $sql =
            "SELECT task_id, card_type, card_type_arg, token, player_id, status FROM task where status = 'tbd'";
        $tasks = $this->thecrew->getCollectionFromDb($sql);

        $cards = $this->thecrew->cards->getCardsInLocation(
            "trick" . $trick_number
        );
        ...
        foreach ($tasks as $task_id => $task) {
            foreach ($cards as $card_id => $card) {
                if (
                    $task["card_type"] == $card["type"] &&
                    $task["card_type_arg"] == $card["type_arg"]
                ) {
                    $sql =
                    ...
                    if ($task["player_id"] == $trick_winner) {
                        ...
                        $text = clienttranslate(
                            '${player_name} fulfilled task ${value_symbol}${color_symbol}'
                        );
                    } else {
                        $tasks[$task_id]["status"] = "nok";
                        $text = clienttranslate(
                            '${player_name} failed task ${value_symbol}${color_symbol}'
                        );
                    }

                    $sql =
                        "update task set trick = " .
                        $trick_number .
                        ", status = '" .
                    ...
                    $this->thecrew->notifyAllPlayers("taskUpdate", $text, [
                        "task" => $tasks[$task_id],
        ...
        $sql =
            "SELECT task_id, trick, card_type, card_type_arg, token, player_id, status FROM task where token = '1' and status ='tbd'";
        $task1 = $this->thecrew->getObjectFromDB($sql);
        if (
            $task1 != null &&
            $this->thecrew->getUniqueValueFromDB(
        ...
        $sql =
            "SELECT task_id, trick, card_type, card_type_arg, token, player_id, status FROM task where token = '2' and status ='tbd'";
        $task1 = $this->thecrew->getObjectFromDB($sql);
        if (
            $task1 != null &&
            $this->thecrew->getUniqueValueFromDB(
                "select count(*) from task where status = 'ok' and token <> '1'"
            ) > 0
        ) {
            // NOI18N
            $tasksMissed = [];
        ...
        $sql =
            "SELECT task_id, trick, card_type, card_type_arg, token, player_id, status FROM task where token = '3' and status ='tbd'";
        $task1 = $this->thecrew->getObjectFromDB($sql);
        if (
            $task1 != null &&
            $this->thecrew->getUniqueValueFromDB(
                "select count(*) from task where status = 'ok' and token <> '1' and token <> '2' "
            ) > 0
        ) {
            // NOI18N
            $tasksMissed = [];
        ...
        $sql =
            "SELECT task_id, trick, card_type, card_type_arg, token, player_id, status FROM task where token = '4' and status ='tbd'";
        $task1 = $this->thecrew->getObjectFromDB($sql);
        if (
            $task1 != null &&
            $this->thecrew->getUniqueValueFromDB(
                "select count(*) from task where status = 'ok' and token <> '1' and token <> '2' and token <> '3'"
            ) > 0
        ) {
            // NOI18N
            $tasksMissed = [];
        ...
        $sql =
            "SELECT task_id, trick, card_type, card_type_arg, token, player_id, status FROM task where token = '5' and status ='tbd'";
        $task1 = $this->thecrew->getObjectFromDB($sql);
        if (
            $task1 != null &&
            $this->thecrew->getUniqueValueFromDB(
                "select count(*) from task where status = 'ok' and token <> '1' and token <> '2' and token <> '3' and token <> '4'"
            ) > 0
        ) {
            // NOI18N
            $tasksMissed = [];
        ...
        $sql =
            "SELECT task_id, card_type, card_type_arg, token, player_id, status FROM task where token LIKE 'i%' and status = 'ok' order by token";
        $tasks = $this->thecrew->getCollectionFromDb($sql);

        foreach ($tasks as $task_id => $task) {
            $sql =
                "SELECT task_id, card_type, card_type_arg, token, player_id, status FROM task where token LIKE 'i%' and token < '" .
                $task["token"] .
                "' and status = 'tbd'";
        ...
        $sql =
            "SELECT task_id, trick, card_type, card_type_arg, token, player_id, status FROM task where token = 'o' and status ='ok'";
        $task1 = $this->thecrew->getObjectFromDB($sql);
        if ($task1 != null) {
            $sql =
                "SELECT task_id, card_type, card_type_arg, token, player_id, status FROM task where status = 'tbd'";
        ...
        if ($this->specialCheck()) {
            ...
            $this->thecrew->getUniqueValueFromDB("select count(*) from task") >
                0 && // NOI18N
            $this->thecrew->getUniqueValueFromDB(
                "select count(*) from task where status <> 'ok'"
            ) == 0
    ...
    {
        $this->thecrew->notifyAllPlayers(
            "note",
            clienttranslate('Mission ${nb} completed'),
            [
                "nb" => $this->thecrew->getUniqueValueFromDB(
    ...
    function missionFailed()
    {
        $this->thecrew->notifyAllPlayers(
            "note",
            clienttranslate('Mission ${nb} failed'),
            [
                "nb" => $this->thecrew->getUniqueValueFromDB(
                    "SELECT max(mission) FROM logbook"
                ),
    ...
    function markMissed($tasksMissed)
    {
        foreach ($tasksMissed as $taskmissed_id => $taskmissed) {
            $sql =
                "SELECT player_id, player_name FROM player where player_id = " .
                $taskmissed["player_id"];
            $taskPlayer = $this->thecrew->getObjectFromDB($sql);

            $tasksMissed[$taskmissed_id]["status"] = "nok";

            $sql =
                "update task set status = 'nok' where task_id = " .
                $taskmissed_id;
            $this->thecrew->DbQuery($sql);

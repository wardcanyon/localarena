  ...
  return declare("emppty", ebg.core.gamegui, {
    constructor: function () {
      ...
      this.translatableTexts = {
        ...
        tooltip_energy_description: _(
          "Place 3 energy with matching color on corresponding Maya to harmonize it.",
        ),
        tooltip_inspiration_title: _("Inspiration token"),
        tooltip_inspiration_description: _(
          "Inspiration is used to place energy on a Chakra or channel energy.",
        ),
        tooltip_meditation_title: _("Meditation token"),
        tooltip_meditation_description: _(
          "While meditating, select a meditation token to reveal the corresponding Plenitude token.",
        ),
        tooltip_plenitude_title: _("Plenitude token"),
        tooltip_plenitude_description: _(
          "At the end of the game, you score the number of points written on the plenitude token corresponding to the Chakra color you harmonized.",
        ),
        tooltip_firstplayer_title: _("First player token"),
        tooltip_firstplayer_description: _(
          "The end of the game is triggered when a player has at least five harmonized Chakras at the end of their turn. The current round is finished, allowing all players the same number of turns.",
        ),
        tooltip_meditate_title: _("Meditate"),
        tooltip_meditate_description: _(
          "Click on the meditation token with the desired color to meditate.",
        ),
        tooltip_channel_title: _("Channel energy"),
        tooltip_channel1_description: _(
          "Move one energy down by three Chakras.",
        ),
        tooltip_channel2_description: _(
          "Move three energy down by one Chakra.",
        ),
        tooltip_channel3_description: _(
          "Move one energy down by two Chakras, and another energy by one Chakra.",
        ),
        ...
        tooltip_channel6_description: _(
          "In the desired order, move one energy down and move another energy up by one Chakra.",
        ),
        tooltip_channel7_description: _(
          "Move one energy up OR down by one Chakra.",
        ),
        tooltip_channel8_description: _(
          "Discard one alleviated energy, and then choose one energy from the Universe bag. You must place it in an available Bhagya Bubble.",
        ),
        confirmation_inspirations: _(
          "Are you sure you want to placed your last inspiration token? There is a high risk of you being stuck until the end of the game.",
        ),
        confirmation_meditate: _(
          "You have already meditate on this Chakra. Do you want to continue?",
        ),
      };
    },
    ...
    setup: function (gamedatas) {
      ...
      for (var player_id in gamedatas.players) {
        ...
        if (player.position == 1) {
          dojo.place(
            this.format_block("jstpl_firstplayer", player),
            $("player_board_" + player_id),
          );
          this.addTooltipHtmlToClass(
            "firstPlayerPanel",
            this.format_block("jstpl_tooltip_common", {
              title: this.translatableTexts.tooltip_firstplayer_title,
              description:
                this.translatableTexts.tooltip_firstplayer_description,
            }),
          );
        }
      }
      for (var color in gamedatas.plenitudes) {
        ...
        dojo.addClass(
          "plenitude_" + color,
          "val" + gamedatas.plenitudes[color],
        );
      }
      ...
      dojo
        .query(".energyph")
        .connect("onclick", this, "onEnergyBoardPlaceHolder");
      ...
      this.addTooltipHtmlToClass(
        "inspiration",
        this.format_block("jstpl_tooltip_common", {
          title: this.translatableTexts.tooltip_inspiration_title,
          description: this.translatableTexts.tooltip_inspiration_description,
        }),
      );
      this.addTooltipHtmlToClass(
        "meditation",
        this.format_block("jstpl_tooltip_common", {
          title: this.translatableTexts.tooltip_meditation_title,
          description: this.translatableTexts.tooltip_meditation_description,
        }),
      );
      this.addTooltipHtmlToClass(
        "plenitude",
        this.format_block("jstpl_tooltip_common", {
          title: this.translatableTexts.tooltip_plenitude_title,
          description: this.translatableTexts.tooltip_plenitude_description,
        }),
      );
      this.addTooltipHtmlToClass(
        "firstPlayer1",
        this.format_block("jstpl_tooltip_common", {
          title: this.translatableTexts.tooltip_firstplayer_title,
          description: this.translatableTexts.tooltip_firstplayer_description,
        }),
      );
      this.addTooltipHtmlToClass(
        "imeditatemedit",
        this.format_block("jstpl_tooltip_common", {
          title: this.translatableTexts.tooltip_meditate_title,
          description: this.translatableTexts.tooltip_meditate_description,
        }),
      );
      ...
      for (var i = 1; i <= 8; i++) {
        this.addTooltipHtmlToClass(
          "channel_" + i,
          this.format_block("jstpl_tooltip_common", {
            title: this.translatableTexts.tooltip_channel_title,
            description:
              this.translatableTexts["tooltip_channel" + i + "_description"],
          }),
        );
      }
    ...
    moveEnergy: function (energy) {
      ...
      this.addTooltipHtml(
        ...
        this.format_block("jstpl_tooltip_common", {
          title:
            this.translatableTexts.tooltip_energy_title +
            " : " +
            this.colors[energy.color],
          description: this.translatableTexts.tooltip_energy_description,
        }),
      );
    ...
    moveInspiration: function (inspiration) {
      ...
      var phid =
        "ph_" +
        inspiration.player_id +
        "_" +
        inspiration.location +
        "_" +
        inspiration.location_arg;
      ...
      switch (stateName) {
        case "take":
          $("pagemaintitletext").innerHTML = $(
            ...
            "{receive}",
            '<span class="logIcon blue" ></span> &nbsp;',
          );
          $("pagemaintitletext").innerHTML = $(
            "pagemaintitletext",
          ).innerHTML.replace(
            "{channel}",
            '<span class="logIcon ichannel"></span> &nbsp;',
          );
          $("pagemaintitletext").innerHTML = $(
            "pagemaintitletext",
          ).innerHTML.replace(
            "{meditate}",
            '<span class="logIcon imeditate"></span> &nbsp;',
          );
            ...
            if (args.args.inspirationsLeft > 0 || args.args.frees[1] > 0) {
              ...
              for (var i = 1; i <= 8; i++) {
                if (args.args.channel[i] == 1) {
                  dojo
                    .query("#ph_" + this.player_id + "_channel_" + i)
                    .addClass("selectable");
                }
              }
            }
    ...
    onColor: function (event) {
      ...
      if (this.isCurrentPlayerActive()) {
        if (
          ...
          this.checkAction("actColor")
        ) {
          var color = event.currentTarget.id.split("_")[1];
          if (
            this.stateName == "take" &&
            !dojo.hasClass(
              "meditation_" + this.player_id + "_" + color,
              "hidden",
            )
          ) {
            this.confirmationDialog(
              this.translatableTexts.confirmation_meditate,
              dojo.hitch(this, function () {
                dojo.query(".selectable").removeClass("selectable");
                this.ajaxcall(
                  "/chakra/chakra/actColor.html",
                  {
                    lock: true,
                    color: color,
                  },
        ...
        if (event.currentTarget.classList.contains("selectable")) {
          ...
          if (this.stateName == "take" && this.checkAction("actTake")) {
            if (this.inspirationsNotBlocked > 1 || row == 1) {
              ...
              this.confirmationDialog(
                this.translatableTexts.confirmation_inspirations,
                dojo.hitch(this, function () {
                  dojo.query(".selectable").removeClass("selectable");
                  dojo.query(".selected").removeClass("selected");
                  this.ajaxcall(
        ...
        if (event.currentTarget.classList.contains("selectable")) {
          if (this.stateName == "take" && this.checkAction("actTake")) {
            ...
            if (event.currentTarget.classList.contains("selected")) {
              if (col != 1)
                dojo.query("#maya .col1 .energy").removeClass("selected");
              if (col != 2)
                dojo.query("#maya .col2 .energy").removeClass("selected");
              if (col != 3)
                dojo.query("#maya .col3 .energy").removeClass("selected");
              ...
              if (
                dojo.query("#maya .col" + col + " .energy.black").length > 0 &&
                dojo.query("#maya .col" + col + " .energy.black.selected")
                  .length < 1
              ) {
                dojo
                  .query("#maya .col" + col + " .energy.black")[0]
    ...
    notif_newenergy: function (notif) {
      ...
      if (dojo.byId(id) == null) {
        dojo.place(
          this.format_block("jstpl_energy", energy),
          $("color_" + energy.color),
        );
      }
    ...
    notif_newmeditation: function (notif) {
      dojo.removeClass(
        "meditation_" + notif.args.player_id + "_" + notif.args.color,
        "hidden",
      );
      dojo.addClass(
        "meditation_" + notif.args.player_id + "_" + notif.args.color,
    ...
    notif_reveal: function (notif) {
      for (var color in notif.args.plenitudes) {
        ...
        dojo.addClass(
          "plenitude_" + color,
          "val" + notif.args.plenitudes[color],
        );
      }
    },
    ...
    getTokenDiv: function (key, args) {
      ...
      switch (key) {
        ...
        case "meditation":
          ret +=
            '<span class="logIcon meditation ' +
            args[key] +
            '" title="' +
            this.colors[args[key]] +
            '"></span>';
          break;
        case "colorcanal":
          ret +=

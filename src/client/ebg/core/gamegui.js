...
(function (factory) {
    ...
    var EbgCoreGamegui = (function () {
        ...
        EbgCoreGamegui.prototype.updateMultiactive = function (multiactive) {
            if (multiactive === undefined) {
                this.multiactive = [];
            }
            else {
                this.multiactive = multiactive.map(function (x) { return parseInt(x); });
            }
        };
        EbgCoreGamegui.prototype.notif_gameStateMultipleActiveUpdate = function (notif) {
            console.log('** got notif: gameStateMultipleActiveUpdate');
            console.log(notif.args);
            this.updateMultiactive(notif.args);
            this.updateUiForState();
        };
        EbgCoreGamegui.prototype.notif_gameStateChange = function (notif) {
            var notifArgs = notif.args;
            if (this.bgg_stateId > 0) {
                var state_1 = this.bgg_states[this.bgg_stateId];
                this.onLeavingState(state_1["name"]);
            }
        ...
        EbgCoreGamegui.prototype.updateUiForState = function () {
            ...
            if (this.isCurrentPlayerActive()) {
                dojo.byId("pagemaintitletext").innerHTML = this.format_string_recursive(_(state["descriptionmyturn"]), state.args);
            }
            else {
                dojo.byId("pagemaintitletext").innerHTML = this.format_string_recursive(_(state["description"]), state.args);
            }
        };
        ...
        EbgCoreGamegui.prototype.divActPlayer = function () {
            var player_id = this.active_player;

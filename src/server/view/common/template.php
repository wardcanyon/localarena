...
class Template
{
    ...
    function pparse($handle)
    {
        ...
        if (
            !isset($this->compiled_code[$handle]) ||
            empty($this->compiled_code[$handle])
        ) {
            // Actually compile the code now.
            $this->compiled_code[$handle] = $this->compile(
    ...
    function make_filename($filename)
    {
        ...
        if (substr($filename, 0, 1) != "/") {
            $filename = ($rp_filename = realpath($this->root . "/" . $filename))
                ? $rp_filename
                : $filename;
        }
    ...
    function loadfile($handle)
    {
        ...
        if (
            isset($this->uncompiled_code[$handle]) &&
            !empty($this->uncompiled_code[$handle])
        ) {
            return true;
        }
        ...
        for ($i = 0; $i < $line_count; $i++) {
            ...
            if (preg_match("#<!-- BEGIN (.*?) -->#", $code_lines[$i], $m)) {
                for ($j = $i++; $j < $line_count; $j++) {
                    if (
                        preg_match("#<!-- END (.*?) -->#", $code_lines[$j], $n)
                    ) {
                        break;
                    }

                    $code_lines[$j] = preg_replace(
                ...
                if (preg_match("#<!-- END (.*?) -->#", $code_lines[$i], $n)) {
                    ...
                    if ($block_nesting_level < 2) {
                        ...
                        $code_lines[$i] =
                            '$_' .
                            $n[1] .
                            '_count = ( isset($this->_tpldata[\'' .
                            $n[1] .
                            '.\']) ) ?  sizeof($this->_tpldata[\'' .
                            $n[1] .
                            '.\']) : 0;';
                        $code_lines[$i] .=
                            "\n" .
                        ...
                        $varref = $this->generate_block_data_ref(
                            $namespace,
                            false
                        );

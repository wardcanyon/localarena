...
export default class EbgStock {
  ...
  addToStockWithId(type, id, from = null) {
    ...
    if (this.selectionMode != 0) {
      $(this.container_div.id + "_item_" + id).classList.add(
        "stockitem_selectable",
      );
    }
  ...
  onClick(event) {
    ...
    if (event.currentTarget.classList.contains("stockitem_selectable")) {
      ...
      if (this.onChangeSelection != null) {
        var itemId = event.currentTarget.id.substr(
          (this.container_div.id + "_item_").length,
        );
  ...
  isSelected(id) {
    return $(this.container_div.id + "_item_" + id).classList.contains(
      "stockitem_selected",
    );
  }
  ...
  unselectItem(id) {
    $(this.container_div.id + "_item_" + id).classList.remove(
      "stockitem_selected",
    );
  }

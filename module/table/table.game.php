...
class Table {
        ...
        foreach ($lines as $line) {
            ...
            if (substr($line, 0, 2) == '--' || $line == '')
                ...
                if (substr(trim($line), -1, 1) == ';') {
                    // Perform the query
                    $this->conn->query($templine) or print('Error performing query \'<strong>' . $templine . '\': ' . $this->conn->error . '<br /><br />');
    ...
    {
        $players = array();
        $players[2317679] = ['player_no' => 1, 'player_id' => 2317679, 'player_canal' => '5ee02c466b5611e99d3379755f289b56', 'player_name' => 'Mistergos0', 'player_avatar' => ''];
        $players[2317680] = ['player_no' => 2, 'player_id' => 2317680, 'player_canal' => '5ee02c466b5611e99d3379755f289b56', 'player_name' => 'Mistergos1', 'player_avatar' => ''];
        $players[2317681] = ['player_no' => 3, 'player_id' => 2317681, 'player_canal' => '5ee02c466b5611e99d3379755f289b56', 'player_name' => 'Mistergos2', 'player_avatar' => ''];
    ...
    function initStat($type, $key, $value)
    {
        ...
        if($type == 'player')
        {
           ...
           foreach($players as $player)
           {
               $this->DbQuery('INSERT INTO `stats`(`stats_type`, `stats_player_id`, `stats_value`) VALUES ('.$id.','.$player['player_id'].','.$value.')');
           }
        }
        else
        {
    ...
    function saveState()
    {
        ...
        foreach($players as $player)
        {
            ...
            $sql = 'INSERT INTO `replay`(`replay_move_id`, `replay_player_id`,`replay_gamedatas` ) VALUES ('.$moveId.','.$this->getCurrentPlayerId().',\''.json_encode($this->getAllDatas()).'\')';
            $this->DbQuery($sql);
        }
    ...
    {
        $gameinfos = array();
        $gameinfos['player_colors'] = array( "ff0000", "008000", "0000ff", "ffa500", "773300" );
        return $gameinfos;
    }
}

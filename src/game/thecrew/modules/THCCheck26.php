...
class THCCheck26 extends THCCheck
{
    ...
    function specialCheck()
    {
        ...
        if ($best_value == 1 && $winningColor < 5) {
            $this->thecrew->setGameStateValue(
                "check_count",
                $this->thecrew->getGameStateValue("check_count") + 1
            );
        }

        $nbLeft = thecrew::getUniqueValueFromDB(
            "select count(*) from card where card_type_arg = 1 and card_type <> '5' and (card_location = 'hand' or card_location='comm')"
        ); // NOI18N
        ...
        if ($this->thecrew->getGameStateValue("check_count") >= 2) {
            ...
            $this->isLastTrick() ||
            $nbLeft + $this->thecrew->getGameStateValue("check_count") < 2
        ) {
            $this->missionFailed();
        } else {

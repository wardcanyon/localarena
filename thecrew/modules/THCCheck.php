...
{    
    ...
    function check()
    {
        ...
        if($task1 != null && $this->thecrew->getUniqueValueFromDB("select count(*) from task where status = 'ok'") > 0)// NOI18N
        {
            $tasksMissed = array();
        ...
        $task1 =  $this->thecrew->getObjectFromDB( $sql );
        if($task1 != null && $this->thecrew->getUniqueValueFromDB("select count(*) from task where status = 'ok' and token <> '1'") > 0)// NOI18N
        {
            $tasksMissed = array();
            $tasksMissed[$task1['task_id']] = $task1;
            $this->markMissed($tasksMissed);
        ...
        $task1 =  $this->thecrew->getObjectFromDB( $sql );
        if($task1 != null && $this->thecrew->getUniqueValueFromDB("select count(*) from task where status = 'ok' and token <> '1' and token <> '2' ") > 0)// NOI18N
        {
            $tasksMissed = array();
            $tasksMissed[$task1['task_id']] = $task1;
            $this->markMissed($tasksMissed);
        ...
        $task1 =  $this->thecrew->getObjectFromDB( $sql );
        if($task1 != null && $this->thecrew->getUniqueValueFromDB("select count(*) from task where status = 'ok' and token <> '1' and token <> '2' and token <> '3'") > 0)// NOI18N
        {
            $tasksMissed = array();
            $tasksMissed[$task1['task_id']] = $task1;
            $this->markMissed($tasksMissed);
        ...
        $task1 =  $this->thecrew->getObjectFromDB( $sql );
        if($task1 != null && $this->thecrew->getUniqueValueFromDB("select count(*) from task where status = 'ok' and token <> '1' and token <> '2' and token <> '3' and token <> '4'") > 0)// NOI18N
        {
            $tasksMissed = array();
            $tasksMissed[$task1['task_id']] = $task1;
            $this->markMissed($tasksMissed);
        ...
        else if($this->thecrew->getUniqueValueFromDB("select count(*) from task")>0 // NOI18N
            && $this->thecrew->getUniqueValueFromDB("select count(*) from task where status <> 'ok'") == 0)// NOI18N
        {
            $this->missionSuccess();
        }
        else if($this->thecrew->getUniqueValueFromDB("select count(*) from task where status = 'nok'") > 0// NOI18N
            || $this->isLastTrick())
        {            
            $this->missionFailed();
        }
    ...
    function missionSuccess()
    {
        $this->thecrew->notifyAllPlayers('note', clienttranslate('Mission ${nb} completed'), array(
            'nb' =>  $this->thecrew->getUniqueValueFromDB( "SELECT max(mission) FROM logbook")
        ));

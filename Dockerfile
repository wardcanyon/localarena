...
# Install PCOV for fast code-coverage reports. The extension is loaded but
# DISABLED by default (`pcov.enabled=0`), so it adds zero overhead to normal
# test runs; PCOV is a no-op while disabled. Enable it per-invocation with
# `php -d pcov.enabled=1 ...` (see the "Generating code-coverage reports"
# section of README.md). We append the setting to the file generated by
# `docker-php-ext-enable` so it is applied immediately after the extension is
# loaded.

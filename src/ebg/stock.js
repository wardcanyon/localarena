define([
    "dojo","dojo/_base/declare","dojo/dom-geometry", "dojo/fx", "dojo/dom-style",
],
function (dojo, declare,  domGeom, fx, style) {
    return declare("ebg.stock", null, {
        ...
        {
        	this.itemTypes[type] = {type: type, weight: weight, image:image, image_position:image_position};
        },
        ...
        addToStockWithId: function( type, id, from = null )
        {
        	...
        	var backy = -Math.floor(item.image_position / this.image_items_per_row) * this.item_height;

        	this.items.push({id:id, type:type});
        	const propComparator = (types) =>  (a, b) => types[a.type].weight - types[b.type].weight;        	
        	...
        	var template = '<div id="'+this.container_div.id+'_item_'+id+'" class="stockitem" style="opacity:0; top: '+position.top+'px; left: '+position.left+'px; width: '+this.item_width+'px; height: '+this.item_height+'px; background-image: url('+item.image+'); background-position: '+backx+'px '+backy+'px; "></div>';
        	dojo.place(template, this.container_div);
        	if(this.selectionMode != 0)
    		{
        		dojo.query("#"+this.container_div.id+'_item_'+id).addClass("stockitem_selectable");    			
    		}
        	...
        	{
        		dojo.fadeIn({ duration: 1000, node:this.container_div.id+'_item_'+id}).play();
        	}
       ...
       onClick:function(event)
       {
        ...
        {
        	this.items = this.items.filter(function(value, index, arr){ return value.id != id;});
        	if(to != null)
        	{
                var start = domGeom.position(this.container_div.id+'_item_'+id);
                ...
                dojo.animateProperty({
	                ...
	                onEnd: dojo.partial( function(tnode) {
	                	...
	                	animation.onEnd = dojo.partial( function(tnode) {dojo.destroy(tnode);}, tnode),
	        			animation.play();
		            }, tnode),
    	       	  }).play();
        	}
        ...
        getPosition: function(index)
        {
        	var computedStyle = style.getComputedStyle(this.container_div);
     		...
     		var left = (index % itemsInLine) * (this.item_margin+this.item_width-this.horizontalOverlap);
     		...
     		if(this.centerItems)
     		{
     			left += (width - nb_in_current_line * (this.item_margin+this.item_width-this.horizontalOverlap))/2;
     		}
        ...
        {      
        	...
        	for(var idx in this.items)
        	{
        		...
        		var left = (i % itemsInLine) * (this.item_margin+this.item_width-this.horizontalOverlap);
        		var top = line * (this.item_margin+this.item_height-this.verticalOverlap);
        		var nb_in_current_line = Math.min(itemsInLine, this.items.length - itemsInLine * line);
        ...
        setSelectionMode: function(mode)
        {
    		...
    		dojo.query("#"+this.container_div.id+" .stockitem").removeClass("stockitem_selectable stockitem_selected");
    		
    		if(this.selectionMode != 0)
    		{
        		dojo.query("#"+this.container_div.id+" .stockitem").addClass("stockitem_selectable");    			
    		}
        },
        ...
        {
    		dojo.query("#"+this.container_div.id+'_item_'+id).hasClass("stockitem_selected");  
        },
        ...
        selectItem: function(id)
        {
        	...
        	dojo.query("#"+this.container_div.id+'_item_'+id).addClass("stockitem_selected");  
        },

        unselectItem: function(id)
        {
    		dojo.query("#"+this.container_div.id+'_item_'+id).removeClass("stockitem_selected");
        },

        unselectAll: function()
        {
    		dojo.query("#"+this.container_div.id+' .stockitem_selected').removeClass("stockitem_selected");
        },
        ...
        getSelectedItems: function()
        {
        	...
        	var items= this.items;
        	 dojo.query("#"+this.container_div.id+' .stockitem_selected').forEach(function(node, index, arr){
        ...
        getUnselectedItems: function()
        {
        	var ret = [];
        	var items= this.items;
        	 dojo.query("#"+this.container_div.id+' .stockitem:not(.stockitem_selected)').forEach(function(node, index, arr){
	         	  var split = node.id.split('_');
		          var id = split[split.length - 1]; 
		          for(var idx in items)
		          {
        ...
        getAllItems: function()
        {
        	var ret = [];
        	var items= this.items;
        	 dojo.query("#"+this.container_div.id+' .stockitem').forEach(function(node, index, arr){
	         	  var split = node.id.split('_');
		          var id = split[split.length - 1]; 
		          for(var idx in items)
		          {

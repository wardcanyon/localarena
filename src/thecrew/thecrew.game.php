...
class thecrew extends Table
{
    ...
    function actButton($button)
    {
        ...
        switch($this->gamestate->state()['name'])
        {
            case 'endMission':
                if($button == 'yes')
                {
                    self::notifyAllPlayers('note', clienttranslate('${player_name} wants to continue'),array(
    ...
    function actFinishComm($place)
    {
        ...
        $sql = "update player set comm_token = '".$place."' where player_id=".$player_id;
        self::DbQuery( $sql );
    ...
    function argPickCrew()
    {
        ...
        $result['players'] = self::getCollectionFromDb( $sql );
        foreach($result['players'] as $player_id => $player) {
            if(self::getGameStateValue('commander_id') != $player_id || $mission['id'] == 50)
            {
    ...
    function stPreparation()
    {
        ...
        $sql = "update player set card_id = NULL, comm_token = 'middle', player_trick_number = 0";
        self::DbQuery( $sql );
    ...
    {
        $this->gamestate->changeActivePlayer(self::getGameStateValue('comm_id'));
        $this->gamestate->nextState('next');
    }

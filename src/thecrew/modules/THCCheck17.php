    ...
    function specialCheck()
    {
        ...
        $still9 =
            thecrew::getUniqueValueFromDB(
                "select count(*) from card where card_type_arg = 9 and ( card_location = 'hand' or card_location = 'cardontable')"
            ) > 0; // NOI18N
        ...
        if (
            ...
            ($this->isLastTrick() || !$still9) &&
            thecrew::getUniqueValueFromDB("select count(*) from task") > 0 && // NOI18N
            thecrew::getUniqueValueFromDB(
                "select count(*) from task where status <> 'ok'"
            ) == 0
        ) {

...
define([
    "dojo","dojo/_base/declare", "dojo/dom-geometry", "dojo/fx", "dojo/dom-style", "dijit/Tooltip", "dojo/fx/easing",
...
function (dojo, declare, domGeom, fx, style, Tooltip) {
    return declare("ebg.core.gamegui", null, {
        ...
        divYou : function() {
            ...
            var color_bg = "";
            if (this.gamedatas.players[this.player_id] && this.bg_game_players[this.player_id].color_back) {
                color_bg = "background-color:#" + this.bg_game_players[this.player_id].color_back + ";";
            }
        ...
        addLog: function(event)
        {
        	if(event.notification_log != "")
    		{
	            ...
	            dojo.place(this.format_block('jstpl_bg_game_message',{id: event.gamelog_id, content: raw}), "bg_game_logs", "first");
	            
	            dojo.query("#bg_game_message_"+event.gamelog_id).connect('onclick', this, dojo.partial(
	            		function(id) { 
	            			var action_id = id;
	            			this.confirmationDialog( _("Do you want to replay the game from this point?"), function() {	   
	            				
					            	var newurl = window.location.href.split('?')[0]+"?replayFrom="+action_id;
        ...
        slideToObject: function( mobile_obj, target_obj, duration=500, delay=0 )
        {
             return this.slideToObjectPos(mobile_obj, target_obj,0,0, duration, delay);           
        },
        
        slideToObjectPos: function( mobile_obj, target_obj, target_x, target_y, duration=500, delay=0 )
        {        	
        ...
        {
        	var animation = dojo.fadeOut({ duration: duration, delay:delay, node:node});
        	animation.onEnd = dojo.partial( function(tnode) {dojo.destroy(tnode);}, node),
			animation.play();
        },
        ...
        addTooltip: function( nodeId, helpString, actionString, delay=400)
        {
        	new Tooltip({
                connectId: [nodeId],
                label: helpString !== undefined?helpString:"" + actionString !== undefined?actionString:"",
                showDelay : delay
            });
        },
        
        addTooltipToClass: function( cssClass, helpString, actionString, delay=400 )
        {
        	new Tooltip({
        ...
        {
        	var ret = !this.lock && this.bgg_states[this.bgg_stateId]['possibleactions'].includes(action_name);
		    if(!ret && !nomessage)
		    {
		    	alert('Impossible action at this state : '+ this.bgg_states[this.bgg_stateId]['name']);
		    }
    		return ret;
        },
        ...
        isPlayerActive(player_id)
        {
        	...
        	return this.active_player_id == player_id || this.multiactive.includes(player_id);
        },
        ...
        addActionButton( id, label, method, destination=null, blinking=false, color="blue" ) {
            ...
            dojo.place(this.format_block('jstpl_bg_game_button', {id:id, color:tcol, label:label, blink:tblink } ), "bg_game_main_buttons"); 

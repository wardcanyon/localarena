...
class emppty extends Table
{
    ...
    protected function setupNewGame($players, $options = [])
    {
        ...
        $sql =
            "INSERT INTO inspiration (player_id, id, location, location_arg) VALUES ";
        $values = [];
        foreach ($players as $player_id => $player) {
            for ($i = 1; $i <= 5; $i++) {
                $values[] =
        ...
        for ($row = 1; $row <= 3; $row++) {
            for ($col = 1; $col <= 3; $col++) {
                if ($this->getEnergy("maya", $row, $col) == null) {
                    $energy = self::getObjectFromDB(
                        "SELECT id, color, location, row, col FROM energy where location ='bag' limit 1"
                    );
                    if ($energy != null) {
                        self::DbQuery(
                            "update energy set location = 'maya', row=" .
                                $row .
                                ", col=" .
                                $col .
                                " where id = " .
    ...
    {
        $value = self::getUniqueValueFromDB(
            "SELECT value FROM plenitude where color='" . $color . "'"
        );
        self::DbQuery(
            "update player set " .
                $color .
    ...
    function getEnergy($location, $row, $col)
    {
        return self::getObjectFromDB(
            "SELECT id, color, location, row, col FROM energy where location ='maya' and row =" .
                $row .
                " and col=" .
                $col
        ); // NOI18N
    ...
    {
        return self::getObjectFromDB(
            "SELECT id, color, location, row, col FROM energy where id=" . $id
        );
    }
    function getEnergies($location, $row = null, $col = null)
    {
        $sql =
            "SELECT id, color, location, row, col FROM energy where location ='" .
            $location .
            "'";
        if ($row != null) {
            $sql = $sql . " and row =" . $row;
    ...
    function isHarmonized($player_id, $row)
    {
        ...
        return self::getUniqueValueFromDB(
            "SELECT count(*) FROM energy where location='" .
                $player_id .
                "' and row = " .
                $row .
                " and color = '" .
    ...
    protected function getAllDatas()
    {
        ...
        $sql =
            "SELECT player_id id, player_score score, player_no position, if(purple>0,1,0) purple, if(darkblue>0,1,0) darkblue, if(blue>0,1,0) blue, if(green>0,1,0) green, if(yellow>0,1,0) yellow, if(orange>0,1,0) orange, if(red>0,1,0) red FROM player ";
        $result["players"] = self::getCollectionFromDb($sql);

        foreach ($result["players"] as $player_id => $player) {
            $sql =
                "SELECT player_id, id, location, location_arg FROM inspiration where player_id=" .
                $player_id;
        ...
        $sql =
            "SELECT purple, darkblue, blue, green, yellow, orange, red FROM player where player_id=" .
            self::getCurrentPlayerId();
        $result["plenitudes"] = self::getObjectFromDb($sql);

        $sql =
            "SELECT id, color, location, row, col FROM energy where location <> 'bag'";
    ...
    function actColor($color)
    {
        ...
        $sql =
            "SELECT player_id id, player_name, player_color FROM player where player_id=" .
            $player_id;
        $player = self::getObjectFromDb($sql);

        if ($this->gamestate->state()["name"] == "take") {
            //move back inspirations
            $maxBoard = self::getUniqueValueFromDB(
                "SELECT max(location_arg) FROM inspiration where player_id =" .
                    $player_id .
                    " and location = 'board'"
            ); // NOI18N
            ...
            $inspirations = self::getObjectListFromDB(
                "SELECT id FROM inspiration where player_id =" .
                    $player_id .
                    " and location = 'channel'"
            ); // NOI18N

            foreach ($inspirations as $c => $insp) {
                $maxBoard++;
                self::DbQuery(
                    "update inspiration set location_arg=" .
                        $maxBoard .
                        ", location='board' where id = " .
                        $insp["id"] .
                        " and player_id =" .
                        $player_id
                );
            ...
            self::notifyAllPlayers("newmeditation", "", [
                "player_id" => $player_id,
                "color" => $color,
                "value" => 0,
            ]);

            self::notifyPlayer($player_id, "newmeditation", "", [
                "player_id" => $player_id,
                "color" => $color,
                "value" => $value,
            ]);
            ...
            self::notifyAllPlayers(
                "notee",
                clienttranslate('${player_name} meditates on ${meditation}'),
                [
            ...
            //remove black energy
            $energyId = self::getUniqueValueFromDB(
                "SELECT id FROM energy where location='" .
                    $player_id .
                    "' and row = 9 order by col desc limit 1"
            );
            self::DbQuery("delete from energy where id = " . $energyId);

            self::notifyAllPlayers("destruct", "", [
                "id" => "energy_" . $energyId,
            ]);
            ...
            $energyId = self::getUniqueValueFromDB(
                "SELECT id FROM energy where location='bag' and color='" .
                    $color .
                    "' limit 1"
            );
            ...
            self::DbQuery(
                "update energy set location = '" .
                    $player_id .
                    "', row= " .
                    $row .
                    ", col= " .
                    array_shift($emptyCol) .
                    " where id = " .
                    $energyId
            );

            $sql =
                "SELECT id, color, location, row, col FROM energy where id=" .
                $energyId;
            $energy = self::getObjectFromDB($sql);
            self::notifyAllPlayers("newenergy", "", [
                "energy" => $energy,
            ]);
            ...
            self::notifyAllPlayers(
                "notee",
                clienttranslate(
                    '${player_name} chooses a new energy ${colorcanal}'
                ),
                [
                    "player_name" => $player["player_name"],
                    "colorcanal" => $color,
    ...
    function actMove($energyId, $row)
    {
        ...
        $player_id = self::getActivePlayerId();
        $sql =
            "SELECT player_id id, player_name, player_color, player_score FROM player where player_id=" .
            $player_id;
        $player = self::getObjectFromDb($sql);
        ...
        if (
            array_key_exists($energyId, $possibles) &&
            in_array($row, $possibles[$energyId])
        ) {
            ...
            if ($channel == 8) {
                ...
                if (
                    ...
                    $inspirationId = self::getUniqueValueFromDB(
                        "SELECT id FROM inspiration where player_id =" .
                            $player_id .
                            " and location = 'chakra' and location_arg = " .
                            ($row - 1)
                    ); // NOI18N
                    if (
                        $inspirationId != null &&
                    ...
                    self::DbQuery(
                        "update player set player_score = player_score + 1 where player_id =" .
                            $player_id
                    );
                    ...
                    self::notifyAllPlayers(
                        "harmonize",
                        clienttranslate(
                            '${player_name} harmonizes its ${color} Chakra'
                        ),
                        [
                            "player_name" => $player["player_name"],
                ...
                if (count($this->channels[$channel][0]) > $step + 1) {
                    self::setGameStateValue(
                        "alreadyMoved",
                        self::getGameStateValue("alreadyMoved") * 100 +
                            $energyId
                    );
                    self::setGameStateValue("step", $step + 1);
                    self::setGameStateValue(
                        "choice",
                        array_search($row, $possibles[$energyId])
                    );
    ...
    function actCancel()
    {
        ...
        if (self::getGameStateValue("step") == 0) {
            ...
            self::notifyAllPlayers(
                "notee",
                clienttranslate('${player_name} cancels'),
                [
                    "player_name" => $player["player_name"],
                ]
            );
            ...
            $inspirationId = self::getUniqueValueFromDB(
                "SELECT id FROM inspiration where player_id =" .
                    $player_id .
                    " and location = 'channel' and location_arg = " .
                    $channel_id
            ); // NOI18N
    ...
    function actChannel($id)
    {
        ...
        self::notifyAllPlayers(
            "notee",
            clienttranslate('${player_name} channels energy'),
            [
                "player_name" => $player["player_name"],
            ]
        );
        ...
        $inspirationId = self::getUniqueValueFromDB(
            "SELECT id FROM inspiration where player_id =" .
                $player_id .
                " and location = 'board' order by location_arg desc limit 1"
        ); // NOI18N
        self::DbQuery(
            "update inspiration set location_arg=" .
                $id .
                ", location='channel' where id = " .
                $inspirationId .
                " and player_id =" .
                $player_id
        );
    ...
    function actTake($energyIds, $row)
    {
        ...
        if (count($energies) == 0 || count($energies) > count($emptyCol)) {
                ...
                if (in_array($energy["color"], $colors)) {
                    ...
                    if ($energy["col"] != $column) {
                        throw new feException(
                            "all energy must come from same maya flow"
                        );
                    }
                }
            }
            ...
            if (
                self::getUniqueValueFromDB(
                    "SELECT count(*) FROM energy where location ='maya' and color='black' and col=" .
                        $column
                ) > 0 &&
                !in_array("black", $colors)
            ) {
            ...
            if (
                self::getUniqueValueFromDB(
                    "SELECT count(*) FROM inspiration where player_id =" .
                        $player_id .
                        " and location = 'chakra' and location_arg=" .
                        ($row - 1)
                ) > 0
            ...
            self::notifyAllPlayers(
                "notee",
                clienttranslate('${player_name} takes ${energies}'),
                [
                    "player_name" => $player["player_name"],
                    "energies" => $colors,
                ]
    ...
    function notifyInspiration($inspirationId)
    {
        $player_id = self::getActivePlayerId();
        $sql =
            "SELECT id, player_id, location, location_arg FROM inspiration where id=" .
            $inspirationId .
            " and player_id=" .
            $player_id;
        $inspiration = self::getObjectFromDB($sql);

        self::notifyAllPlayers("inspiration", "", [
    ...
    function argTake()
    {
        ...
        $return = [];
        $frees = [];
        $left = self::getUniqueValueFromDB(
            "SELECT count(*) FROM inspiration where player_id=" .
                $player_id .
                " and location='board'"
        ); // NOI18N
    ...
    function isChannelPossible($channel)
    {
        ...
        if ($channel == 8) {
            if (
                ...
                $colors = self::getCollectionFromDB(
                    "SELECT distinct(color) FROM energy where location = 'bag'"
                ); // NOI18N
        ...
        foreach ($possibleMoves as $moveIndex => $moves) {
            ...
            for ($step = 0; $step < count($moves); $step++) {
                ...
                foreach ($prePossibles as $alreadyMoved => $tablepre) {
                    foreach ($energies as $id => $energy) {
                        $nextState = $this->getNextTableState(
                            $tablepre,
                            $channel,
                            $alreadyMoved,
                            $energy,
    ...
    function getNextTableState($table, $channel, $am, $energy, $diff)
    {
        ...
        if (
            !$harmonized[$energy["row"]] &&
            !in_array($energy["id"], $alreadyMoved) &&
            $energy["row"] != 9
        ) {
            $possible = true;
            $harmo = 0;
            ...
            for ($d = 1; $d <= abs($diff) && $possible; $d++) {
                $rowTested =
                    $energy["row"] + ($d + $harmo) * ($diff > 0 ? 1 : -1);

                if ($rowTested >= 10 || $rowTested <= 0) {
                    $possible = false;
                } elseif ($harmonized[$rowTested]) {
                    $harmo++;
                    $d--;
                } else {
    ...
    function argChannel()
    {
        ...
        $ret["channel"] = $channel;
        $ret["step"] = $step;
        $ret["inspiration"] = self::getUniqueValueFromDB(
            "SELECT id FROM inspiration where player_id ='" .
                $player_id .
                "' and location = 'channel' and location_arg=" .
                $channel
    ...
    function stCheckFinish()
    {
        ...
        if (!$finished) {
            $sql =
                "SELECT player_id id, player_name, player_color FROM player ";
            $players = self::getCollectionFromDb($sql);
            foreach ($players as $player_id => $player) {
                $nb = 0;
                for ($row = 2; $row <= 8; $row++) {
                ...
                if ($nb >= 5) {
                    ...
                    self::notifyAllPlayers(
                        "notee",
                        clienttranslate(
                            '${player_name} has harmonized five Chakras. This game will end at the end of this round.'
                        ),
                        [
                            "player_name" =>
        ...
        if (
            $finished &&
            self::getUniqueValueFromDB(
                "SELECT player_no FROM player where player_id=" .
            ...
            foreach ($this->colors as $cid => $color) {
                if ($color != "black") {
                    $value = self::getUniqueValueFromDB(
                        "SELECT value FROM plenitude where color='" .
                            $color .
                            "'"
                    );
                    self::DbQuery(
                        "update player set " . $color . " = " . $value
                    );
                }
            }

            $sql =
                "SELECT purple, darkblue, blue, green, yellow, orange, red FROM player where player_no = 1";
            ...
            //End of the game
            $sql =
                "SELECT player_id id, player_name, player_color, player_score FROM player ";
            $players = self::getCollectionFromDb($sql);
            $harmo_max = 1;
            $player_harmo_max = -1;
            foreach ($players as $player_id => $player) {
            ...
            foreach ($players as $player_id => $player) {
                ...
                $nbBlack = self::getUniqueValueFromDB(
                    "SELECT count(*) FROM energy where location='" .
                        $player_id .
                        "' and row = 9 and color = 'black'"
                ); // NOI18N
                ...
                if ($players[$player_id]["harmo"] == $harmo_max) {
                    ...
                    self::notifyAllPlayers(
                        "notee",
                        clienttranslate(
                            '${player_name} wins 2 points with harmonization bonus'
                        ),
                        [
                            "player_name" =>
                                $players[$player_id]["player_name"],

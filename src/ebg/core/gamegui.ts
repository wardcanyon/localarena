...
export class EbgCoreGamegui {
  ...
  completesetup(gamename, gamedatas) {
    ...
    dojo.subscribe(
      ...
      this,
      "notif_gameStateMultipleActiveUpdate",
    );
  ...
  divActPlayer() {
    ...
    if (
      ...
      color_bg =
        "background-color:#" + this.bg_game_players[player_id].color_back + ";";
    }
    var you =
      '<span style="font-weight:bold;color:#' +
      color +
      ...
      dojo.query("#bg_game_message_" + event.gamelog_id).connect(
        ...
        dojo.partial(function (id) {
          ...
          this.confirmationDialog(
            ...
            function () {
              var newurl =
                window.location.href.split("?")[0] + "?replayFrom=" + action_id;
  ...
  slideToObject(mobile_obj, target_obj, duration = 500, delay = 0) {
  ...
  // translation."
  addTooltipToClass(cssClass, helpString?, actionString?, delay = 400) {
    new Tooltip({
